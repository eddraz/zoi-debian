...
### Fixes
- **Weather chip stuck on …:** `widgets/Weather.qml` shadowed the `Commons.Weather` singleton, so the bar never read `ready`/`label` and the fetch never mounted. Widget imports `../Commons` as `Commons`; `shell.qml` force-instantiates Weather; Process restarts via a 0ms timer; `qs-weather` uses `/usr/bin/python3` and falls back to Bogotá if ip-api fails. Docs: configuration, features, architecture, troubleshooting, install, skill. `install.sh` checks `python3` + `qs-weather`.
