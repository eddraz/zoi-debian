...
- Don't use Yazi `[open]` `name = "*.md"` (Yazi 26 wants `url` or `mime`). Don't add Nerd Fonts for Yazi icons; keep ASCII in `yazi-theme.toml.tpl`. Don't install JetBrainsMono Nerd Font — bar icons are Canvas; XDG default browser is Mullvad/Thorium. Brave Origin uses the official `curl -fsS https://dl.brave.com/install.sh | FLAVOR=origin sh`; do not add Brave desktop files to the XDG default loop.
- Third-party bins (Yazi, Mullvad, Brave Origin, Inlyne, Node.js, Zed, `gh`, Deno, Bun, pnpm, Voxtype, Bruno, Etcher, cloudflared) are arch-gated or official installers in `install.sh`. Pin APT lists with `arch=$ARCH`. Node is **nodejs.org latest tarball → `/usr/local`**, not Debian `nodejs`. GitHub CLI keyring must match the pinned SHA256.
