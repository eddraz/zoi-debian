...
- Third-party bins (Yazi, Mullvad, Inlyne, Node.js, Zed, `gh`, Deno, Bun, pnpm, Voxtype, Bruno, Etcher, cloudflared) are arch-gated or official installers in `install.sh`. Pin APT lists with `arch=$ARCH`. Node is **nodejs.org latest tarball → `/usr/local`**, not Debian `nodejs`. GitHub CLI keyring must match the pinned SHA256.
